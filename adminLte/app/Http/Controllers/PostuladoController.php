...
class PostuladoController extends Controller
{
    ...
    public function importar(Request $request)
    {
        if ($request->hasFile('import_file')) {
            $file = $request->file('import_file');
            $import = new PostuladoImport();
            Excel::import($import, $file);

            // Obtén los postulados importados
            $postulados = $import->get();

            // Itera los postulados importados
            foreach ($postulados as $postulado) {
                // Verifica si el postulante ya existe en la base de datos por correo electrónico
                $existingPostulado = Postulado::where('mail', $postulado->mail)->first();

                if (!$existingPostulado) {
                    // Si el postulante no existe, crea un nuevo registro
                    $newPostulado = new Postulado();
                    $newPostulado->nombre = $postulado->nombre;
                    $newPostulado->genero = $postulado->genero;
                    $newPostulado->mail = $postulado->mail;
                    $newPostulado->telefono = $postulado->telefono;
                    $newPostulado->save();
                }
            }
        }
